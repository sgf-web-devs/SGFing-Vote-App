...
namespace AlmostEngine.Screenshot
{
	public class ScreenshotNameParser
	{
		...
		public static string ParseSymbols (string name, ScreenshotResolution resolution, string currentLayer = "")
		{
		...
		/// </summary>
		public static string ParseFileName (string screenshotName, ScreenshotResolution resolution, DestinationFolder destination, string customPath, TextureExporter.ImageFileFormat format, bool overrideFiles, string currentLayer = "")
		{
			string filename = "";
			...
			// File name
			filename += ParseSymbols (screenshotName, resolution, currentLayer);

			// Get the file extension
			string extension = ParseExtension (format);

			// Increment the file number if a file already exist
			if (!overrideFiles && File.Exists (filename + extension)) {
				int i = 1;
