	...
	public class ScreenshotConfig
	{
		...
		public void UpdateResolutionFilenames (List<ScreenshotResolution> resolutions, string currentLayer = "")
		{
			foreach (ScreenshotResolution resolution in resolutions) {
				UpdateFileName (resolution, currentLayer);
			}
		}
		...
		{
			string path = m_DestinationFolder == ScreenshotNameParser.DestinationFolder.CUSTOM_FOLDER ? m_RootedPath : m_RelativePath;
			return ScreenshotNameParser.ParseFileName (m_FileName, resolution, m_DestinationFolder, path, m_FileFormat, m_OverrideFiles, currentLayer);
		}

		public void UpdateFileName (ScreenshotResolution resolution, string currentLayer = "")
		{
			resolution.m_FileName = ParseFileName (resolution, currentLayer);
		}
